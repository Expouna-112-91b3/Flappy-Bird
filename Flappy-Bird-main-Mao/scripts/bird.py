...
class Bird:
    def __init__(self):
        ...
        self.__y = 400 # alterar aqui!!

        self.__handlastseen = 400
        self.__movi = 400
    ...
    def handControl(self, hands):
        ...
        self.__movi = handpos.handidf(hands)

        if not self.__movi:
            return
        
        direction = self.__movi - self.__handlastseen

        if self.__y >= self.__ground_height:
            self.__y = self.__ground_height

            
        if self.__movi > self.__handlastseen:
            
            self.__y += abs(direction)
        
        else:

            self.__y -= direction
            print(self.__y, direction)

        self.__handlastseen = self.__movi
